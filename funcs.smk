...
def check_no_whitespace(values, description):
    ...
    Barcodes, wells, and serum replicates are joined by a space to key the QC-drop YAML
    files, which `aggregate_qc_drops` splits back apart, and serum replicates are also
    directory names under ``results/sera``. Whitespace in any of them would silently
